...
from datalabs.arrow_reader import (
    ArrowReader,
...
from datalabs.arrow_writer import ArrowWriter, BeamWriter
from datalabs.data_files import DataFilesDict, sanitize_patterns
...
from datalabs.info import DatasetInfo, DatasetInfosDict, PostProcessedInfo
from datalabs.iterable_dataset import (
    _generate_examples_from_tables_wrapper,
    ExamplesIterable,
    IterableDataset,
)
from datalabs.naming import camelcase_to_snakecase, filename_prefix_for_split
...
class BuilderConfig:
        ...
        for invalid_char in invalid_windows_characters:
            if invalid_char in self.name:
                raise InvalidConfigName(
                    f"Bad characters from black list '{invalid_windows_characters}' "
                    f"found in '{self.name}'. "
                    f"They could create issues when creating a directory "
                    f"for this config on Windows filesystem."
                )
    ...
    def create_config_id(
        ...
        However the name of a config is not sufficient to have a unique
         identifier for the dataset being generated
        since it doesn't take into account:
        - the config kwargs that can be used to overwrite attributes
        - the custom features used to write the dataset
        - the data_files for json/text/csv/pandas datalab
        Therefore the config id is just the config name with an optional
        suffix based on these.
        """
        # Possibly add a suffix to the name to handle custom
        # features/data_files/config_kwargs
        suffix: Optional[str] = None
        config_kwargs_to_add_to_suffix = config_kwargs.copy()
        # name and version are already used to build the cache directory
        config_kwargs_to_add_to_suffix.pop("name", None)
        config_kwargs_to_add_to_suffix.pop("version", None)
        # data dir handling (when specified it points to the manually
        # downloaded data):
        # it was previously ignored before the introduction of config
        # id because we didn't want
        # to change the config name. Now it's fine to take it into
        # account for the config id.
...
class DatasetBuilder:
    ...
    **Configuration**: Some `DatasetBuilder`s expose multiple variants of the
    ...
    def __init__(
        ...
        **config_kwargs,
        ...
        Args:
            ...
            name: `str` name, optional configuration for the dataset that
             affects the data generated on disk. Different
                `builder_config`s will have their own subdirectories and versions.
                If not provided, uses the first configuration in self.BUILDER_CONFIGS
            hash: a hash specific to the dataset code. Used to update the
            caching directory when the dataset loading
                script code is updated (to avoid reusing old data).
                The typical caching directory (defined in ``self._relative_data_dir``)
                 is: ``name/version/hash/``
            base_path: `str`, base path for relative paths that are used to download
             files. This can be a remote url.
            features: `Features`, optional features that will be used to read/write
             the dataset
                It can be used to changed the :obj:`datalab.Features` description of
                 a dataset for example.
            use_auth_token (:obj:`str` or :obj:`bool`, optional): Optional string or
             boolean to use as Bearer token
                for remote files on the Datasets Hub. If True, will get token from
                ``"~/.huggingface"``.
            namespace: `str`, used to separate builders with the same name but not
             coming from the same namespace.
                For example to separate "squad" from "lhoestq/squad" (the builder
                 name would be "lhoestq___squad").
            data_files: for builders like "csv" or "json" that need the user to
            specify data files. They can be either
                local or remote files. For convenience you can use a DataFilesDict.
            data_dir: `str`, for builders that require manual download. It must be
             the path to the local directory containing
        ...
        # Prepare config: DatasetConfig contains name, version and description but
        # can be extended by each dataset
        if (
            "features"
        ...
        self._cache_downloaded_dir = (
            os.path.join(cache_dir, config.DOWNLOADED_DATASETS_DIR)
            if cache_dir
        ...
        lock_path = os.path.join(
            self._cache_dir_root, self._cache_dir.replace(os.sep, "_") + ".lock"
        )
        with FileLock(lock_path):
            if os.path.exists(self._cache_dir) and os.path.exists(
                "dataset_info.json"
            ):  # check if data exist
                if len(os.listdir(self._cache_dir)) > 0:
                    logger.info("Overwrite dataset info from restored data version.")
                    self.info = DatasetInfo.from_directory(self._cache_dir)
                else:  # dir exists but no data, remove the empty dir as data
                    # aren't available anymore
                    logger.warning(
                        f"Old caching folder {self._cache_dir} for dataset "
                        f"{self.name} exists but not data were found. Removing it. "
                    )
    ...
    def get_all_exported_dataset_infos(cls) -> dict:
        ...
        dset_infos_file_path = os.path.join(
            cls.get_imported_module_dir(), config.DATASETDICT_INFOS_FILENAME
        )
    ...
    def get_exported_dataset_info(self) -> DatasetInfo:
        ...
        return self.get_all_exported_dataset_infos().get(
            self.config.name, DatasetInfo()
        )

    def _create_builder_config(
        self, name=None, custom_features=None, **config_kwargs
    ) -> Tuple[BuilderConfig, str]:
        """Create and validate BuilderConfig object as well as a unique
        config id for this config.
        Raises ValueError if there are multiple builder configs and name
         and DEFAULT_CONFIG_NAME are None.
        ...
        if name is None and self.BUILDER_CONFIGS and not config_kwargs:
            if self.DEFAULT_CONFIG_NAME is not None:
                ...
                logger.warning(
                    f"No config specified, defaulting"
                    f" to: {self.name}/{builder_config.name}"
                )
            else:
                # if len(self.BUILDER_CONFIGS) > 1:
                #     example_of_usage = f"load_dataset('{self.name}',
                #     '{self.BUILDER_CONFIGS[0].name}')"
                #     raise ValueError(
                ...
                logger.info(
                    f"No config specified, defaulting to "
                    f"first: {self.name}/{builder_config.name}"
                )
        ...
        if isinstance(name, str):
            ...
            if builder_config is None and self.BUILDER_CONFIGS:
                raise ValueError(
                    f"BuilderConfig {name} not found. Available:"
                    f" {list(self.builder_configs.keys())}"
                )

        # if not using an existing config, then create a new
        # config on the fly with config_kwargs
            ...
            for key, value in config_kwargs.items():
                if value is not None:
                    if not hasattr(builder_config, key):
                        raise ValueError(
                            f"BuilderConfig {builder_config} "
                            f"doesn't have a '{key}' key."
                        )
        ...
        if is_custom:
            ...
            if builder_config != self.builder_configs[builder_config.name]:
                raise ValueError(
                    "Cannot name a custom BuilderConfig the same as"
                    " an available "
                    f"BuilderConfig. Change the name. Available "
                    f"BuilderConfigs: {list(self.builder_configs.keys())}"
                )
            if not builder_config.version:
                raise ValueError(
                    f"BuilderConfig {builder_config.name} must " f"have a version"
                )
            # if not builder_config.description:
            #     raise ValueError(f"BuilderConfig
            #     {builder_config.name} must have a description"  )
    ...
    def _relative_data_dir(self, with_version=True, with_hash=True) -> str:
        ...
        If any of these element is missing or if ``with_version=False``
         the corresponding subfolders are dropped.
        """
        builder_data_dir = (
            self.name if self.namespace is None else f"{self.namespace}___{self.name}"
        )
    ...
    def _build_cache_dir(self):
        ...
        if version_dirs:
            ...
            if other_version != self.config.version:
                warn_msg = (
                    f"Found a different version {str(other_version)} "
                    f"of dataset {self.name} in "
                    f"cache_dir {self._cache_dir_root}. Using currently"
                    f" defined version "
    ...
    def download_and_prepare(
        ...
        **download_and_prepare_kwargs,
        ...
        Args:
            download_config (Optional ``datalab.DownloadConfig``:
            specific download configuration parameters.
            download_mode (Optional `datalab.GenerateMode`): select the
             download/generate mode - Default to REUSE_DATASET_IF_EXISTS
            ignore_verifications (bool): Ignore the verifications
            ...
            try_from_hf_gcs (bool): If True, it will try to download the
             already prepared dataset from the Hf google cloud storage
            dl_manager (Optional ``datalab.DownloadManager``): specific
            Download Manger to use
            base_path ( Optional ``str``): base path for relative paths
            that are used to download files. This can be a remote url.
                If not specified, the value of the ``base_path`` attribute
                 (``self.base_path``) will be used instead.
            use_auth_token (Optional ``Union[str, bool]``): Optional string
             or boolean to use as Bearer token for remote files on the Datasets Hub.
                If True, will get token from ~/.huggingface.

        """
        download_mode = GenerateMode(
            download_mode or GenerateMode.REUSE_DATASET_IF_EXISTS
        )
        ...
        with FileLock(lock_path):
            ...
            ):  # if cache dir is local, check for available space
                if not utils.has_sufficient_disk_space(
                    self.info.size_in_bytes or 0, directory=self._cache_dir_root
                ):
                    raise IOError(
                        f"Not enough disk space. Needed: "
                        f"{utils.size_str(self.info.size_in_bytes or 0)} (download"
                        f": {utils.size_str(self.info.download_size or 0)}, generated"
                        f": {utils.size_str(self.info.dataset_size or 0)}, "
                        f"post-processed:"
                        f" {utils.size_str(self.info.post_processing_size or 0)})"
                    )
            ...
            if self.info.size_in_bytes:
                logger.info(
                    ...
                    f" {utils.size_str(self.info.dataset_size)}, "
                    f"post-processed: "
                    f"{utils.size_str(self.info.post_processing_size)}, "
                    f"total: {utils.size_str(self.info.size_in_bytes)}) "
            ...
            with incomplete_dir(self._cache_dir) as tmp_data_dir:
                # Temporarily assign _cache_dir to tmp_data_dir to avoid
                # having to forward
                ...
                with utils.temporary_assignment(self, "_cache_dir", tmp_data_dir):
                    ...
                    if try_from_hf_gcs:
                        try:
                            ...
                            logger.info(
                                "Dataset not on Hf google storage. Downloading"
                                " and preparing it from source"
                            )
                        except ConnectionError:
                            logger.warning(
                                "HF google storage unreachable. Downloading and"
                                " preparing it from source"
                            )
    ...
    def _check_manual_download(self, dl_manager):
        if (
            ...
            raise ManualDownloadError(
                textwrap.dedent(
                    f"""The dataset {self.name} with config {self.config.name}requires
                        manual data.
                    Please follow the manual download instructions:
                     {self.manual_download_instructions}Manual data can be loaded with:datalab.load_dataset({self.name}, data_dir='<path/to/manual/data>')"""  # noqa
                )
            )
        ...
        for split in self.info.splits:
            for resource_file_name in self._post_processing_resources(split).values():
                if os.sep in resource_file_name:
                    raise ValueError(
                        f"Resources shouldn't be in a "
                        f"sub-directory: {resource_file_name}"
                    )
                try:
                    resource_path = utils.cached_path(
                        remote_cache_dir + "/" + resource_file_name
                    )
                    shutil.move(
                    ...
                    logger.info(
                        f"Couldn't download resourse file"
                        f" {resource_file_name} from Hf google storage."
                    )
            ...
            for resource_name, resource_file_name in self._post_processing_resources(
                ...
                if not os.path.exists(resource_path):
                    ...
                    if downloaded_resource_path:
                        logger.info(
                            f"Downloaded post-processing "
                            f"resource {resource_name} as {resource_file_name}"
                        )
    ...
    def _save_infos(self):
        ...
        with FileLock(lock_path):
            DatasetInfosDict(**{self.config.name: self.info}).write_to_directory(
                self.get_imported_module_dir()
            )
    ...
    def as_dataset(
        ...
        Args:
            ...
            run_post_process (bool, default=True): Whether to run
             post-processing dataset transforms and/or add
                indexes.
            ignore_verifications (bool, default=False): Whether to
            ignore the verifications of the
        ...
        if not os.path.exists(self._cache_dir):
            raise AssertionError(
                (
                    f"Dataset {self.name}: could not find data "
                    f"in {self._cache_dir_root}. Please make sure to call "
        ...
        logger.debug(
            f'Constructing Dataset for split {split or ", ".join(self.info.splits)}, '
            f"from {self._cache_dir}"
        )
    ...
    def _build_single_dataset(
        ...
        if run_post_process:
            ...
            resources_paths = {
                ...
                for resource_name, resource_file_name in self._post_processing_resources(  # noqa
                    split
                ).items()
            }
            ...
            if post_processed is not None:
                ...
                self.info.post_processing_size = sum(
                    ...
                    for split_checksums_dicts in self.info.post_processed.resources_checksums.values()  # noqa
                ...
                if self.info.post_processed.features is not None:
                    if self.info.post_processed.features.type != ds.features.type:
                        raise ValueError(
                            f"Post-processed features info don't "
                            f"match the dataset:\nGot"
    ...
    def _get_dataset_fingerprint(self, split: Union[ReadInstruction, Split]) -> str:
        """The dataset fingerprint is the hash of the relative
        directory dataset_name/config_name/version/hash, as well as the split specs."""
        hasher = Hasher()
    ...
    def _download_post_processing_resources(
        ...
        """Download the resource using the download manager
...
class ArrowBasedBuilder(DatasetBuilder):
    """Base class for datalab with data generation based on Arrow
    loading functions (CSV/JSON/Parquet)."""
    ...
    def _get_examples_iterable_for_split(
        ...
        return ExamplesIterable(
            _generate_examples_from_tables_wrapper(self._generate_tables),
...
class BeamBasedBuilder(DatasetBuilder):
    ...
    def _make_split_generators_kwargs(self, prepare_split_kwargs):
        ...
        split_generators_arg_names = inspect.signature(
    ...
    def _download_and_prepare(self, dl_manager, verify_infos):
        ...
        if not beam_runner and not beam_options:
            usage_example = (
                f"load_dataset('{self.name}', '{self.config.name}',"
                f" beam_runner='DirectRunner')"
            )
    ...
    def _prepare_split(self, split_generator, pipeline):
        ...
        _ = (
            pipeline | split_name >> _build_pcollection()
