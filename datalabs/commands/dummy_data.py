...
class DummyDataGeneratorDownloadManager(DownloadManager):
    ...
    def auto_generate_dummy_data_folder(
        ...
        for src_path, relative_dst_path in zip(
            self.downloaded_dummy_paths, self.expected_dummy_paths
        ):
    ...
    def _create_dummy_data(
        ...
        if os.path.isfile(src_path):
            ...
            is_line_by_line_text_file = any(
                extension in dst_path_extensions
                for extension in line_by_line_extensions
            )
            ...
            # xml file
                ...
                extension in dst_path_extensions for extension in [".xml", ".txm"]
            ):
                if xml_tag is None:
                    logger.warning(
                        "Found xml file but 'xml_tag' is set to None."
                        " Please provide --xml_tag"
                    )
                else:
                    self._create_xml_dummy_data(
                        src_path, dst_path, xml_tag, n_lines=n_lines, encoding=encoding
                    )
                return 1
            logger.warning(
    ...
    @staticmethod
    def _create_xml_dummy_data(
        src_path, dst_path, xml_tag, n_lines=5, encoding=DEFAULT_ENCODING
    ):
        Path(dst_path).parent.mkdir(exist_ok=True, parents=True)
        with open(src_path, "r", encoding=encoding) as src_file:
            n_line = 0
...
class DummyDataCommand(BaseDatasetsCLICommand):
    ...
    def register_subcommand(parser: ArgumentParser):
        ...
        test_parser.add_argument(
            ...
            type=str,
            default=None,
            help="Optional, json field to read the data from when auto-generating "
            "dummy data. In the json data files, this field must point to a "
            "list of samples as json objects (ex: the 'data' field for"
            " squad-like files)",
        )
        test_parser.add_argument(
            "--xml_tag",
            type=str,
            default=None,
            help="Optional, xml tag name of the samples inside the xml files when"
            " auto-generating dummy data.",
        )
        test_parser.add_argument(
            "--match_text_files",
            type=str,
            default=None,
            help="Optional, a comma separated list of file patterns that looks for"
            " line-by-line text files other than *.txt or *.csv. Example:"
            " --match_text_files *.label",
        )
        test_parser.add_argument(
            "--keep_uncompressed",
            action="store_true",
            help="Whether to leave the dummy data folders uncompressed when"
            " auto-generating dummy data. Useful for debugging for to do "
        ...
        test_parser.add_argument(
            ...
            type=str,
            default=None,
            help="Cache directory to download and cache files when"
            " auto-generating dummy data",
        )
        test_parser.add_argument(
            "--encoding",
            type=str,
            default=None,
            help=f"Encoding to use when auto-generating dummy data."
            f" Defaults to {DEFAULT_ENCODING}",
        )
        test_parser.add_argument(
            "path_to_dataset",
            type=str,
            help="Path to the dataset (example: ./datalab/squad)",
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            for builder_config in builder_configs:
                ...
                dataset_builder = builder_cls(
                    name=name, hash=dataset_module.hash, cache_dir=tmp_dir
                )
            ...
            if self._auto_generate and not self._keep_uncompressed:
                if all(auto_generate_results):
                    print(
                        f"Automatic dummy data generation succeeded "
                        f"for all configs of '{self._path_to_dataset}'"
                    )
                else:
                    print(
                        f"Automatic dummy data generation failed for"
                        f" some configs of '{self._path_to_dataset}'"
                    )
    ...
    def _autogenerate_dummy_data(
        ...
        if not keep_uncompressed:
            ...
            try:
                ...
                logger.error(
                    f"Failed to load dummy data for config "
                    f"'{dataset_builder.config.name}''.\nOriginal error:\n" + str(e)
                )
                return False
            else:
                if all(n_examples > 0 for n_examples in n_examples_per_split.values()):
                    logger.warning(
                        f"Dummy data generation done and dummy data test"
                        f" succeeded for config '{dataset_builder.config.name}''."
                    )
                    return True
                else:
                    empty_splits = [
                    ...
                    logger.warning(
                        f"Dummy data generation done but dummy data test "
                        f"failed since splits {empty_splits} have 0 examples"
                        f" for config '{dataset_builder.config.name}''."
                    )
                    return False
        else:
            ...
            logger.info(
                f"Dummy data generated in directory '{generated_dummy_data_dir}'"
                f" but kept uncompressed. "
                "Please compress this directory into a zip file to "
                "use it for dummy data tests."
            )
    ...
    def _print_dummy_data_instructions(self, dataset_builder, mock_dl_manager):
        ...
        try:
            ...
            print(
                f"Dataset {self._dataset_name} with config {mock_dl_manager.config} "
                f"seems to already open files in the method `_split_generators(...)`."
                f" You might consider to instead only open files in the method "
                f"`_generate_examples(...)` instead. If this is not possible the "
                f"dummy data has to be created with less guidance. Make sure you"
                f" create the file {e.filename}."
            )
        ...
        for split in generator_splits:
            ...
            try:
                ...
                config_string = (
                    f"config {mock_dl_manager.config.name} of "
                    if mock_dl_manager.config is not None
                    else ""
                )
                dummy_data_guidance_print += (
                    "- In order to create the dummy data for "
                    + config_string
                    + f"{self._dataset_name}, please go into the folder "
                    f"'{dummy_data_folder}' with `cd {dummy_data_folder}` . \n\n"
                )
                ...
                dummy_data_guidance_print += (
                    f"- It appears that the function"
                    f" `_generate_examples(...)` expects "
                    f"one or more files in the folder "
                    f"{dummy_file_name} using the function "
                    f"`glob.glob(...)`. In this case, please"
                    f" refer   `_generate_examples(...)`"
                    f" method to see under which filename "
                    f"the dummy data files should"
                    f" be created. \n\n"
                )
        ...
        if len(files_to_create) > 0:
            ...
            if (
                ...
                dummy_data_guidance_print += (
                    f"- Please create a single "
                    f"dummy data file called "
                    f"'{next(iter(files_to_create))}' "
                    f"from the folder '{dummy_data_folder}'."
                    f" Make sure that the dummy data file "
                    f"provides at least one example for"
                    f" the split(s) '{split_names}' \n\n"
                )
                files_string = dummy_file_name
            else:
                files_string = ", ".join(files_to_create)
                dummy_data_guidance_print += (
                    f"- Please create the following "
                    f"dummy data files '{files_string}' "
                    f"from  folder '{dummy_data_folder}'\n\n"
                )

                dummy_data_guidance_print += (
                    f"- For each of the splits"
                    f" '{split_names}', make sure that "
                    f"one or more of the dummy data files "
                    f"provide at least one example \n\n"
                )

            dummy_data_guidance_print += (
                f"- If the method `_generate_examples(...)` "
                f"includes multiple `open()` statements,"
                f" you might have to create other files"
                f" in addition to '{files_string}'. "
                f"In this case please refer to the "
                f"`_generate_examples(...)` method \n\n"
            )

        if len(files_to_create) == 1 and next(iter(files_to_create)) == dummy_file_name:
            dummy_data_guidance_print += (
                f"- After the dummy data file is "
                f"created, it should be zipped to "
                f"'{dummy_file_name}.zip' with the"
                f" command `zip {dummy_file_name}.zip"
                f" {dummy_file_name}` \n\n"
            )

            dummy_data_guidance_print += (
                f"- You can now delete the file "
                f"'{dummy_file_name}' with the command"
                f" `rm {dummy_file_name}` \n\n"
            )

            dummy_data_guidance_print += (
                f"- To get the file '{dummy_file_name}' "
                f"back for further changes to the dummy"
                f" data, simply unzip {dummy_file_name}.zip"
                f" with the command `unzip "
                f"{dummy_file_name}.zip` \n\n"
            )
        else:
            dummy_data_guidance_print += (
                f"- After all dummy data files are "
                f"created, they should be zipped "
                f"recursively to '{dummy_file_name}.zip'"
                f" with the command `zip -r"
                f" {dummy_file_name}.zip {dummy_file_name}/`"
                f" \n\n"
            )

            dummy_data_guidance_print += (
                f"- You can now delete the folder "
                f"'{dummy_file_name}' with the command "
                f"`rm -r {dummy_file_name}` \n\n"
            )

            dummy_data_guidance_print += (
                f"- To get the folder '{dummy_file_name}'"
                f" back for further changes to the"
                f" dummy data, simply unzip "
                f"{dummy_file_name}.zip with the "
                f"command `unzip {dummy_file_name}.zip` \n\n"
            )

        dummy_data_guidance_print += (
            f"- Make sure you have created the"
            f" file '{dummy_file_name}.zip'"
            f" in '{dummy_data_folder}' \n"
        )
