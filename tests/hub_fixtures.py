...
ENDPOINT_STAGING_DATASETS_URL = ENDPOINT_STAGING + "/datalab/{path}/resolve/{revision}/{name}"
...
def hf_private_dataset_repo_txt_data(hf_private_dataset_repo_txt_data_):
    with patch("datalab.config.HF_ENDPOINT", ENDPOINT_STAGING):
        with patch("datalab.config.HUB_DATASETS_URL", ENDPOINT_STAGING_DATASETS_URL):
