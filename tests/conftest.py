...
def set_test_cache_config(tmp_path_factory, monkeypatch):
    ...
    test_hf_datasets_cache = test_hf_cache_home / "datalab"
    test_hf_metrics_cache = test_hf_cache_home / "metrics"
    test_hf_modules_cache = test_hf_cache_home / "modules"
    monkeypatch.setattr("datalab.config.HF_DATASETS_CACHE", str(test_hf_datasets_cache))
    monkeypatch.setattr("datalab.config.HF_METRICS_CACHE", str(test_hf_metrics_cache))
    monkeypatch.setattr("datalab.config.HF_MODULES_CACHE", str(test_hf_modules_cache))
    test_downloaded_datasets_path = test_hf_datasets_cache / "downloads"
    monkeypatch.setattr("datalab.config.DOWNLOADED_DATASETS_PATH", str(test_downloaded_datasets_path))
    test_extracted_datasets_path = test_hf_datasets_cache / "downloads" / "extracted"
    monkeypatch.setattr("datalab.config.EXTRACTED_DATASETS_PATH", str(test_extracted_datasets_path))
...
def set_update_download_counts_to_false(monkeypatch):
    ...
    monkeypatch.setattr("datalab.config.HF_UPDATE_DOWNLOAD_COUNTS", False)
