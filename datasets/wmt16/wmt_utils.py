...
_TRAIN_SUBSETS = [
    ...
    SubDataset(  # pylint:disable=g-complex-comprehension
        ...
        target="en",
        sources={"zh"},
        url="https://huggingface.co/datasets/wmt/wmt18/resolve/main/cwmt-wmt/%s.zip"
        % ss,
...
class Wmt(ABC, datalabs.GeneratorBasedBuilder):
    ...
    def _info(self):
        ...
        features_sample = datalabs.Features(
            ...
            # {
            #     "translation_column": datalabs.Value("string"),
            #     "lang_sub_columns": datalabs.Value("string"),
            # }
        )
    ...
    def _split_generators(self, dl_manager):
        ...
        for ss_name in itertools.chain.from_iterable(self.subsets.values()):
            ...
            if dataset.get_manual_dl_files(source):
                ...
                # +++++++++++++++++++++
                logger.info(
                    "Skipping {dataset.name} for now. Incomplete dataset for {self.config.name}"
                )
                continue
                # +++++++++++++++++++++
