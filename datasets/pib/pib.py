...
class PibConfig(datalabs.BuilderConfig):
    """BuilderConfig for PIB"""

    def __init__(self, language_pair, version=datalabs.Version("1.3.0"), **kwargs):
...
class Pib(datalabs.GeneratorBasedBuilder):
    ...
    def _info(self):
        return datalabs.DatasetInfo(
            ...
            #     {"translation": datalabs.features.Translation(languages=[self.config.src, self.config.tgt])}
            # ),
            features=datalabs.Features(
                {
    ...
    def _generate_examples(self, archive):
        ...
        for idx, (s, t) in enumerate(zip(src, tgt)):
            data = {
                    "source_lang": self.config.src,
                    "source_text": s,
                    "target_lang": self.config.tgt,
                    "target_text": t,
                }
