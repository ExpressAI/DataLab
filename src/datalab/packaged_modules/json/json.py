...
import datalab
from datalab.utils.file_utils import readline
...
@dataclass
class JsonConfig(datalab.BuilderConfig):
...
class Json(datalab.ArrowBasedBuilder):
    ...
    def _cast_classlabels(self, pa_table: pa.Table) -> pa.Table:
        if self.config.features:
            ...
            for i, col in enumerate(self.config.features.keys()):
                if isinstance(self.config.features[col], datalab.ClassLabel):
