...
class S3FileSystem(s3fs.S3FileSystem):
    ...
    Examples:
      ...
      >>> import datalab
      >>> s3 = datalab.filesystems.S3FileSystem(anon=True)  # doctest: +SKIP
      ...
      >>> import datalab
      >>> s3 = datalab.filesystems.S3FileSystem(key=aws_access_key_id, secret=aws_secret_access_key)  # doctest: +SKIP
      ...
      >>> import botocore
      >>> from datalab.filesystems import S3Filesystem
      ...
      >>> from datalab import load_from_disk
      >>> from datalab.filesystems import S3Filesystem
      >>>
